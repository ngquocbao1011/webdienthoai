...
<div style="max-width: 800px; margin: 0 auto; background: #fff; padding: 20px;">
    <form action="{{ route('admin.product.update', $product->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div style="margin-bottom: 15px;">
            ...
            <input type="text" name="name" value="{{ $product->name }}" placeholder="Tên sản phẩm.." style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px;">
        </div>

        <div style="margin-bottom: 15px;">
            <label style="display: block; font-weight: bold; margin-bottom: 5px;">Giá</label>
            <input type="number" name="price" value="{{ $product->price }}" placeholder="Giá.." style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px;">
        </div>
        ...
        <div style="margin-bottom: 15px;">
            ...
            <div style="border: 1px solid #ddd; padding: 10px; border-radius: 4px; background: #f9f9f9;">
                @if($product->image)
                    <img src="{{ asset('storage/' . $product->image) }}" alt="Current Image" style="max-width: 100px; margin-bottom: 10px;">
                @endif
        ...
        <div style="margin-bottom: 15px;">
            ...
            <textarea name="description" style="width: 100%; height: 150px; padding: 10px; border: 1px solid #ddd; border-radius: 4px;">{{ $product->description }}</textarea>
        </div>

        <div style="text-align: center;">
            <button type="submit" style="background: #c00080; color: white; border: none; padding: 10px 30px; border-radius: 4px; cursor: pointer; font-weight: bold;">Cập nhật sản phẩm</button>
        </div>
    </form>
</div>
@endsection